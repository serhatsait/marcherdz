    ...
    <div class="row no-gutter">
       <div class="col-xs-12 col-sm-3">
            <div class="panel panel-default">
                <div class="panel-heading"><i class="fa fa-bars" aria-hidden="true"></i> Vitrin Kategorileri</div>
                <div class="panel-body">

                    <?
                    $bugun = date("Y-m-d");
                    $dp = $db->query("SELECT * FROM doping WHERE  (category1 = '$id' or category2 = '$id' or category3 = '$id' or category4 = '$id' or category5 = '$id' or category6 = '$id' or category7 = '$id' or category8 = '$id' or category9 = '$id' or category10 = '$id') and ( name='anasayfa') and (onay = '1') and val >= '$bugun'");
                    ...
                    if (sayac($a->Id) > 0){
                    echo '<li class="maincat"><img src="'.$base_url.'ikonlar/'.$a->ikon.'" alt="'.$a->kategori_adi.'" width="20" height="20" class="absmiddle" /> '.$a->kategori_adi.'</li>';
                    }
                    $ust = $a->Id;
                    $sql2 = $db->query("SELECT * FROM category WHERE ustkategoriId = '$ust'");
                    while ($b = $sql2 ->fetch(PDO::FETCH_OBJ)){
                    if (sayac($b->Id) > 0){
                    echo '<li><a href="anasayfa-ilanlari-'.$b->Id.'-'.slugify($b->kategori_adi).'.html">» '.$b->kategori_adi.' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    }
                    ...
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 3;
                    } else {
                    $u    = $b->ustkategoriId;
                    ...
                    if (sayac($c->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $c->Id . '-' . slugify($c->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $c->kategori_adi . ' <span>( ' . sayac($c->Id) . ' )</span></a></li>';
                    }
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub3"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 4;
                    } else {
                    $u    = $c->ustkategoriId;
                    ...
                    if (sayac($d->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $d->Id . '-' . slugify($d->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $d->kategori_adi . ' <span>( ' . sayac($d->Id) . ' )</span></a></li>';
                    }
                    if (sayac($c->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $c->Id . '-' . slugify($c->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $c->kategori_adi . ' <span>( ' . sayac($c->Id) . ' )</span></a></li>';
                    }
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub3"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub4"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 5;
                    } else {
                    $u    = $d->ustkategoriId;
                    ...
                    if (sayac($e->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $e->Id . '-' . slugify($e->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $e->kategori_adi . ' <span>( ' . sayac($e->Id) . ' )</span></a></li>';
                    }
                    if (sayac($d->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $d->Id . '-' . slugify($d->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $d->kategori_adi . ' <span>( ' . sayac($d->Id) . ' )</span></a></li>';
                    }
                    if (sayac($c->Id) != 0){
                    echo '<li class="sub3"><a href="anasayfa-ilanlari-' . $c->Id . '-' . slugify($c->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $c->kategori_adi . ' <span>( ' . sayac($c->Id) . ' )</span></a></li>';
                    }
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub4"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub5"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 6;
                    } else {
                    $u    = $e->ustkategoriId;
                    ...
                    if (sayac($f->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $f->Id . '-' . slugify($f->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $f->kategori_adi . ' <span>( ' . sayac($f->Id) . ' )</span></a></li>';
                    }
                    if (sayac($e->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $e->Id . '-' . slugify($e->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $e->kategori_adi . ' <span>( ' . sayac($e->Id) . ' )</span></a></li>';
                    }
                    if (sayac($d->Id) != 0){
                    echo '<li class="sub3"><a href="anasayfa-ilanlari-' . $d->Id . '-' . slugify($d->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $d->kategori_adi . ' <span>( ' . sayac($d->Id) . ' )</span></a></li>';
                    }
                    if (sayac($c->Id) != 0){
                    echo '<li class="sub4"><a href="anasayfa-ilanlari-' . $c->Id . '-' . slugify($c->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $c->kategori_adi . ' <span>( ' . sayac($c->Id) . ' )</span></a></li>';
                    }
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub5"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub6"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 7;
                    } else {
                    $u    = $f->ustkategoriId;
                    ...
                    if (sayac($g->Id) != 0){
                    echo '<li class="sub1"><a href="anasayfa-ilanlari-' . $g->Id . '-' . slugify($g->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $g->kategori_adi . ' <span>( ' . sayac($g->Id) . ' )</span></a></li>';
                    }
                    if (sayac($f->Id) != 0){
                    echo '<li class="sub2"><a href="anasayfa-ilanlari-' . $f->Id . '-' . slugify($f->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $f->kategori_adi . ' <span>( ' . sayac($f->Id) . ' )</span></a></li>';
                    }
                    if (sayac($e->Id) != 0){
                    echo '<li class="sub3"><a href="anasayfa-ilanlari-' . $e->Id . '-' . slugify($e->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $e->kategori_adi . ' <span>( ' . sayac($e->Id) . ' )</span></a></li>';
                    }
                    if (sayac($d->Id) != 0){
                    echo '<li class="sub4"><a href="anasayfa-ilanlari-' . $d->Id . '-' . slugify($d->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $d->kategori_adi . ' <span>( ' . sayac($d->Id) . ' )</span></a></li>';
                    }
                    if (sayac($c->Id) != 0){
                    echo '<li class="sub5"><a href="anasayfa-ilanlari-' . $c->Id . '-' . slugify($c->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $c->kategori_adi . ' <span>( ' . sayac($c->Id) . ' )</span></a></li>';
                    }
                    if (sayac($b->Id) != 0){
                    echo '<li class="sub6"><a href="anasayfa-ilanlari-' . $b->Id . '-' . slugify($b->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $b->kategori_adi . ' <span>( ' . sayac($b->Id) . ' )</span></a></li>';
                    }
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub7"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    $sub = 8;
                    }
                    }
                    ...
                    if (sayac($a->Id) != 0){
                    if (sayac($a->Id) != 0){
                    echo '<li class="sub' . $sub . '"><a href="anasayfa-ilanlari-' . $a->Id . '-' . slugify($a->kategori_adi) . '.html'; if ($_GET['daralt'] != ""){ echo '?'.$urlek.''; } echo '">» ' . $a->kategori_adi . ' <span>( ' . sayac($a->Id) . ' )</span></a></li>';
                    }
                    }
                    }
                    echo ' </ul></div>';
...
<a href="javascript:ackapat()" style="margin-bottom:2px;" class="btn btn-danger btn-block visible-xs"><i class="fa fa-filter" aria-hidden="true"></i> Aramayı Filtrele</a>
            <div class="hidden-xs panel panel-default" id="lef">
                ...
                <div class="panel-body">
                    ...
                    if ($id == ""){
                    echo '<form action="anasayfa-ilanlari.html" method="get">';
                    } else {
                    echo ' <form action="anasayfa-ilanlari-'.$id.'-'.slugify($z->kategori_adi).'.html" method="get">';
                    }
...
display:block;
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
			...
			?>
            <div style="padding:5px;"><center>Vitrin <strong>"<? echo $z->kategori_adi; ?>"</strong> Listelemesinde Toplam <span><strong><? echo $toplam22; ?> Vitrin</strong></span> İlanı Bulundu<br></center></div>
            <?
            ...
            <div class="col-xs-9 col-sm-8" style="width: 76.66666667%;"><strong style="font-size:12px">' . $row["title"] . '</strong>
            <br><span style="font-size:12px"><i class="fa fa-clock-o" aria-hidden="true"></i> ' . $e[2] . '-' . $e[1] . '-' . $e[0] . ' <br><i class="fa fa-map-marker" aria-hidden="true"></i> ' . $ilyaz["il_adi"] . ' / ' . $ilceyaz["county_adi"] . '</span>
            ...
            <div class="adv2">
            <div class="image1"><a href="i-'.$row["Id"].'-'.slugify($row["title"]).'.html" alt="'.$row["title"].'" title="'.$row["title"].'"> <div class="acil-box"><img src="' . $src . '" width="93" height="75" alt=""/><div class="box_type" style="margin-top: 5px;background: rgba(0,0,0,0.5);color: #fff;border-radius: 4px;font-weight:600;font-size: 12px;-webkit-border-bottom-left-radius: 3px;-moz-border-radius-bottomleft: 3px;border-bottom-left-radius: 3px;">'.number_format($row[price]).' '.$row[exchange].'</div> </div> </a></div>
			<div class="adv2-title" style="font-size:11px;margin-top:-10px;"><a href="i-'.$row["Id"].'-'.slugify($row["title"]).'.html" alt="'.$row["title"].'" title="'.$row["title"].'">' . tr_ucwords(substr($row["title"],0,20)) . '</a></div>
            </div>
            </div>';
            }
            }
