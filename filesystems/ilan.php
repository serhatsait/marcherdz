...
<!-- 2. Add images to <div class="fotorama"></div>. -->
    ...
    ?>
            ...
            <div class="col-xs-12 col-sm-5">
              <? if ($row["price"] != 0){ ?>
              <center><strong style="font-size:26px;font-family:Verdana;"><? echo number_format($row["price"])." ".$row["exchange"]; ?></strong></center><br>
              <? } ?>
              <center><span style="font-size:12px;font-weight:600;"><i class="fa fa-map-marker" aria-hidden="true"></i> <? echo '<a href="'.$lin[1].'?il='.$ilyaz["id"].'&daralt=1&sayfa=1">' . $ilyaz["il_adi"] . '</a> / <a href="'.$lin[1].'?il='.$ilyaz["id"].'&ilce[]='.$ilceyaz["id"].'&daralt=1&sayfa=1">' . $ilceyaz["county_adi"] . '</a> / <a href="'.$lin[1].'?il='.$ilyaz["id"].'&ilce[]='.$ilceyaz["id"].'&mahalle[]='.$mahalleyaz["id"].'&daralt=1&sayfa=1">' . $mahalleyaz["districtname"] . '</a>'; ?> </span></center>
              ...
              <div class="row font12">
                ...
                <div class="col-xs-7">
                  <? $e = explode("-",$row["dates"]); echo "$e[2]-$e[1]-$e[0]"; ?>
                </div>
              </div>
              <hr class="hr">
              <?
              ...
              <div class="panel panel-default" style="margin-top:10px !important; border-radius:0px !important">
                <div class="panel-heading" style="font-size:12px; background-color:#ffab00 !important"> Kalan Süre : <b><span id="getting-started"></span></b>
                  <?
                ...
                <div class="panel-body">
                <div style="font-size:12px; color:red; padding-bottom:5px">İhale Bitiş Tarihi : <? echo $ee[0]; ?></div>
                  <div style="font-size:14px; font-weight:bold; padding-bottom:8px">Başlangıç: <? echo number_format($row["fiyat2"]); ?> TL</div>
                  <div class="input-group">
                    <input type="text" name="tt" id="tt" class="form-control money" placeholder="0.00 TL" style="font-size:12px">
                    <span class="input-group-btn">
                    <button class="btn btn-danger" type="button" style="font-size: 12px !important;padding: 8px !important;border-bottom: solid 1px #d9534f !important;" onClick="teklif()">Teklif Ver</button>
                    </span> </div>
                  <hr style="margin-top:12px; margin-bottom:0px">
                  <div style="font-size:12px; padding-top:8px">Minimum Artış : <b><? echo number_format($row["fiyat3"]); ?> TL</b></div>
                  <?
				  $sql1 = $db->query("SELECT * FROM teklifler WHERE ilanId = '$id' ORDER BY Id DESC LIMIT 1");
				  $ax1 = $sql1->fetch(PDO::FETCH_ASSOC);
				  ?>
                  <div style="font-size:12px; padding-top:8px">Son Teklif : <b><? echo number_format($ax1["tutar"]); ?> TL</b></div>
                </div>
              </div>
              <? } ?>
            </div>
        ...
        <div class="col-md-4 col-sm-12 col-xs-12">
          <div class="userbox"><strong style="font-size:16px"><center> <i class="fa fa-check" style="color: green;" aria-hidden="true"></i> <? echo $row8["ad_soyad"]; ?></center></strong>
		  <center><strong style="font-size:12px">Üyelik Tarihi : <font color="#636363"><? echo ay2($row8["kayit_tarihi"]); ?></font></strong></center>
		  
            <center><div style="padding-top:5px; color: #113998; font-size:12px; font-weight:600;"> <a href="index.php?page=mesajgonder&id=<? echo $id; ?>" style="color: #113998;"><i class="fa fa-envelope" aria-hidden="true"></i> Mesaj Gönder</a> |
              <? if ($sql9->rowCount() > 0){ ?>
              <a href="http://<? echo $a9["adres"]; ?>.<? echo $murl; ?>" style="color: #113998;"><i class="fa fa-shopping-cart" aria-hidden="true"></i> Mağazaya Git</a> | <a href="index.php?page=profil&id=<? echo $row8["Id"]; ?>" style="padding-top:5px; color: #113998; font-size:12px; font-weight:600;"><i class="fa fa-balance-scale" aria-hidden="true"></i> Profil</a>
              <? } else { ?>
              <a href="<? echo $row8["Id"]; ?>-kullanici-ilanlari.html" style="color: #113998;"><i class="fa fa-user-o" aria-hidden="true"></i> Tüm İlanları</a>
              <? } ?>
            </div></center>
            ...
            <div class="userbox2">
              <div class="row">
                ...
                <div class="col-xs-8"><a href="tel:<? echo $row8["telefon"]; ?>"><font size="2" face="Verdana"><? echo $row8["telefon"]; ?></font></a></div>
              </div>
              <hr class="hr">
              <div class="row">
                <div class="col-xs-4 glyphicon glyphicon-phone"><strong><font size="2" face="Verdana"> Gsm :</font></strong></div>
                <div class="col-xs-8"><a href="tel:<? echo $row8["gsm"]; ?>"><font size="2" face="Verdana"><? echo $row8["gsm"]; ?></font></a></div>
              </div>
            </div>
			 <? } ?>
     <style type="text/css">
...
function fbs_click(width, height) {
    ...
    return false;
			...
			<a href="http://www.facebook.com/share.php?u=<? echo $fbadres; ?>" onClick="return fbs_click(650, 450)" target="_blank" title=""><button style="width:100%; margin-top:10px;" type="button" class="btn btn-facebook "><i class="fa fa-facebook fa-2"></i> Facebook'ta Paylaş</button></a>
<style>
.btn-facebook {
	color: #fff;
...
<center><h6 class="trustSafetyTitle"><font size="2" face="Arial"><img border="0" src="../../img/guvenlik.png" width="24" height="26"><b> Güvenlik İpuçları</b></font></h6><p><font size="2" face="Arial">İlgilendiğiniz aracı veya emlak ilanını görmeden kapora olarak bir ödeme yapmayın, para göndermeyin.</font></p><center>
  ...
  <div class="panel panel-default">
	...
	if ($sqlxx->rowCount() > 0){ ?>
      <table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%" style="font-size:12px">
		...
		if ($x4["il_adi"] == ""){ $x4["il_adi"]= "Belirtilmemiş"; }
		echo '
		<tr>
  ...
  <ul class="tab-menu">
	...
	<br><center><div class="hit" style="font-size:12px"> <strong><i class="fa fa-eye"></i> <? echo $gosterim; ?></strong> Kişi Tarafından Görüntülendi.</div></center>
  <? $sql5 = $db->query("SELECT * FROM groups WHERE modulId = '{$row4['modul']}'"); if ($sql5->rowCount() != 0){ ?>
  <div class="panel panel-default">
    <div class="panel-heading"><i class="fa fa-check-square-o" aria-hidden="true"></i> Özellikler</div>
    <div class="panel-body">
      <br>
	  <style>
...
<iframe class="map-top" width="100%" height="420" src="https://www.google.com/maps/embed/v1/streetview?key=AIzaSyCpxv2EoVBP72pIqOHnzehHqTkWRWCw1Nc&location=<? echo $row["lat"]; ?>, <? echo $row["lng"]; ?>&pitch=5&heading=180&fov=75" allowfullscreen></iframe>
...
<a href="whatsapp://send?text=Merhaba <? echo $row21["title"]; ?>, Başlıklı İlanınız Hakkında Bilgi Alabilirmiyim?&amp;phone=9<? echo $row8["gsm"]; ?>" class="btn btn-dafault button50"><i class="fa fa-whatsapp fa-1x" aria-hidden="true"></i> Whatsapp Mesaj</a>

<a href="tel:<? echo $row8["gsm"]; ?>" class="btn btn-dafault button51"><i class="fa fa-phone fa-1x" aria-hidden="true"></i> İlan Sahibini Ara</a>
