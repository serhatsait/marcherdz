...
<div class="col-xs-12 col-sm-9" style="padding-left:5px;">
			...
			<div class="panel panel-default">
                <div class="panel-heading"><i class="fa fa-thumbs-down" aria-hidden="true"></i> Fiyatı Düşen İlanlar</div>
                <div class="panel-body">
                    <div class="row no-gutter">
                        <?
                        $bugun = date("Y-m-d");
                        $sqlsor = $db->query("SELECT * FROM ilanlar WHERE (bitis >= '$bugun') and confirm = '1' and fiyatdustu = '1' ORDER BY Id DESC LIMIT 24");
						...
						<div class="adv2-title" style="font-size:11px;margin-top:-10px;"><a href="i-'.$row["Id"].'-'.slugify($row["title"]).'.html" alt="'.$row["title"].'" title="'.$row["title"].'">' . ucfirst(substr($row["title"],0,11)) . '..</a></div>
                        </div>
                        </div>';
                        }
