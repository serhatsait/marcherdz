...
<a href="javascript:ackapat()" class="btn btn-danger btn-block visible-xs">Kategori ve Filtreleme</a><br class="visible-xs">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="adv2">
            <div class="image1"><a href="i-'.$row["Id"].'-'.slugify($row["title"]).'.html" alt="'.$row["title"].'" title="'.$row["title"].'"><img src="' . $src . '" alt="'.$row["title"].'" height="75"/></a></div>
			<div class="box_type" style="min-width:60px;text-align: center;bottom: 0;padding: 1px 13px;z-index: 2;background: #696662;color: #fff;font-size:12px;">'.number_format($row[price]).' '.$row[exchange].'</div>
            <div class="adv2-title" style="font-size:11px;margin-top:-10px;"><a href="i-'.$row["Id"].'-'.slugify($row["title"]).'.html" alt="'.$row["title"].'" title="'.$row["title"].'">' . ucfirst(substr($row["title"],0,14)) . '..</a></div>
            </div>
            </div>';
            }
            }
...
$ek .= " and ($vvx) ";	
        ...
        ?>
    <div class="panel panel-default">
        <div class="panel-heading">Kategoriler / <? echo $z->kategori_adi; ?></div>
        <div class="panel-body">
            <div class="row no-gutter">
                <div class="col-xs-8">
            ...
            <hr>
			<? if ($_GET['daralt'] != '') { ?>
            <strong>"<? echo $z->kategori_adi; ?>"</strong> aramanızda <span id="sonuc"></span><br>
            <? } ?>
            <br>
            <?
				$sqls     = $db->query("SELECT * FROM modulitems WHERE modulsId = '$modul'");
            ...
            <div class="col-xs-9 col-sm-8"><strong style="font-size:13px">' . mb_substr($row["title"],0,40) . '..</strong>&nbsp;&nbsp;&nbsp;<img class="hidden-xs" src="img/maps_icon.png" title="Haritalı İlan"/>&nbsp;&nbsp;<img class="hidden-xs" src="img/streetv.png" title="Sokak Görünümü"/>
			...
			echo '</div></div>
            <div class="col-xs-6 col-sm-3 col-md-2" style=""><a href="javascript:void(0)" class="btn btn-danger btn-block price">'; if ($row["type"] == 1 || $row["type"] == 2){ echo 'Get İlan';} else { echo '' . str_replace(",", ".", number_format($row["price"])) . ' ' . $row["exchange"] . ''; }echo '</a></div>
            </div>
            </div>
            ';
            } else {
...
<div id="myModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
	...
	<input type="hidden" name="page" value="hizliarama">
	...
	<input type="hidden" name="daralt" value="1">
	  ...
	  <div class="form-group">
		...
		<select name="zx1" id="zx1" class="form-control" onchange="cat()">
                    <?
			$i = 0;
			$sql = $db ->query("SELECT * FROM category WHERE ustkategoriId = '0' and tip = '0'");
			while ($a = $sql->fetch(PDO::FETCH_ASSOC)){
			if ($i == 0){ $alt = $a["Id"]; } else {}
	  ...
	  <div class="form-group">
		...
		<select name="zx2" id="zx2" class="form-control" >
                    <?
			$i = 0;
			$sql = $db ->query("SELECT * FROM category WHERE ustkategoriId = '$alt'");
			while ($a = $sql->fetch(PDO::FETCH_ASSOC)){
			if ($i == 0){ $alt = $a["Id"]; } else {}
			echo '<option value="'.$a["Id"].'">'.$a["kategori_adi"].'</option>';
			$i++;	  
	 ...
	 <div class="form-group">
	  ...
	  <label class="">Fiyat Aralığı</label>
	   <div class="row no-gutter">
                                <div class="col-xs-6">
                                    <div class="form-group">
                                        <input type="text" name="fiyat1" value="<? echo $_GET["fiyat1"]; ?>" class="form-control money" placeholder="minimum">
	...
	</div><input type="submit" style = "width:100%;" class="btn btn-danger" value="Aramaya Başla">
	</form>
  </div>
        </div>
    </div>