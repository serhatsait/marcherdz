            ...
            <div class="col-sm-9">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <?
                        $uye = $_SESSION["uye"];
                        $sql = $db->query("SELECT * FROM mesajlar WHERE gonderilen = '$uye' and gonderilensil = '0' ORDER BY Id DESC");
                        if ($sql->rowCount() == 0){
                        echo '<div class="alert alert-danger"><strong>Uyarı !</strong> Gelen mesajınız bulunmamaktadır.</div>';
                        } else {
                        ?>
                        ...
                        <table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%" style="font-size:12px">
                            ...
                            <tbody>
                                ...
                                $stil = ' style="font-weight:bold"';
                                } else {
                                $ikon = '<i class="fa fa-envelope-open-o" aria-hidden="true"></i>';
                                $stil = "";
                                }
                                ...
                                <tr>
                                <td>'.$ikon.'</td>
                                <td'.$stil.'><a href="index.php?page=messageread&id='.$row["Id"].'">'.tr_ucwords($c["title"]).'</a></td>
                                <td'.$stil.' class="hidden-xs" align="center"><a href="index.php?page=messageread&id='.$row["Id"].'">'.$b["ad_soyad"].'</a></td>
                                <td'.$stil.' class="hidden-xs" align="center"><a href="index.php?page=messageread&id='.$row["Id"].'">'.$row["tarih"].'</a></td>
                                </tr>
                                ';
                                }
                                ?>
