...
$sql2->execute();
$sql2 = $db->prepare("DELETE FROM images WHERE ilanId = '{$eski}'");
$sql2->execute();
...
$sql56 = $db->query("SELECT uyeId FROM magazalar WHERE uyeId = '$uye'");

if ($sql56->RowCount() > 0 ){ 
$dates    = date("Y-m-d");
$start = date("Y-m-d", strtotime("+$yayintarihi DAYS", strtotime($dates)));
...
$sql->execute(array(null,$uye,$telefon,$mesaj,$data1,$data2,$data3,$data4,$data5,$data6,$data7,$kargolar,$data8,$data9,$data10,$lat,$lng,$zoom,1,$dates,$category1,$category2,$category3,$category4,$category5,$category6,$category7,$category8,$category9,$category10,$yayintarihi,$start,$fiyat2,$fiyat3,$bitis,0));
...
$sql->execute(array(null, "İhale Yayın Ödemesi", $ihalebedeli, $uye, "$lastId Nolu İhale İlanının Yayın Ücreti", 0, $lastId, 0));
if ($eski == ""){
header("location: index.php?page=ihale&id=$lastId");
} else {
header("location: index.php?page=advertsp&err=1");	
}
} else {
header("location: index.php?page=doping&id=$lastId");
}
?>