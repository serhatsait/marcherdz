    ...
    <div class="row no-gutter">
        ...
        <div class="col-xs-12 col-sm-9">
            <div class="panel panel-default">
                <div class="panel-heading"><i class="fa fa-check" aria-hidden="true"></i> Yayında Olan İlanlar</div>
                <div class="panel-body">
                    <?php
                    ...
                    $uye = $_SESSION["uye"];
                    $bugun = date("Y-m-d");
                    $sql = $db->query("SELECT * FROM ilanlar WHERE (uyeId = '$uye' and confirm = '1') and (bitis >= '$bugun') order by Id DESC");
                    if ($sql->rowCount() == 0) {
                        echo '<div class="alert alert-danger" style="margin-bottom:0px !important"><strong>Uyarı:</strong> Yayında olan ilanınız bulunmamaktadır</div>';
                    }
					...
					if ($a["firmadi"] != ""){
						...
						<b>' . $a["title"] . '</b><br><span style="font-size:11px"><b>Eklenme Tarihi :</b> ' . $yayin[2] . '-' . $yayin[1] . '-' . $yayin[0] . ' <b>Kalan Süre :</b> ' . $gun . ' Gün</span><div style = "padding-top:5px"><a href = "'; if ($a["firmadi"] != ""){ echo 'firma'; } else { echo 'i'; } echo '-' . $a["Id"] . '-' . slugify($a["title"]) . '.html" class = "btn btn-success btn-xs">İlana Git</a> '; if ($a["type"] != 2){ if ($a["firmadi"] != ""){ echo '<a href = "index.php?page=add5&id=' . $a["category1"] . '&ilanId=' . $a["Id"] . '" class = "btn btn-success btn-xs" style = "background-color:#337ab7 !important; border-color:#337ab7 !important">Güncelle</a>'; } else { echo '<a href = "index.php?page=guncelle&id=' . $a["category1"] . '&ilanId=' . $a["Id"] . '" class = "btn btn-success btn-xs" style = "background-color:#337ab7 !important; border-color:#337ab7 !important">Güncelle</a> <a href = "index.php?page=fiyatdusur&id=' . $a["category1"] . '&ilanId=' . $a["Id"] . '" class = "btn btn-success btn-xs" style = "background-color:#337ab7 !important; border-color:#337ab7 !important">Fiyatı Düşür</a> '; } } if ($a["firmadi"] == ""){echo ' <a href = "index.php?page=doping&id=' . $a["Id"] . '" class = "btn btn-success btn-xs" style = "background-color:#337ab7 !important; border-color:#337ab7 !important">Doping Yap</a> ';} if ($a["type"] != 2){ echo ' <a href = "javascript:void(0)" onclick = "ilansil(' . $a["Id"] . ')" class = "btn btn-success btn-xs" style = "background-color:red!important;
						border-color:red!important">Sil</a>'; } echo '</div></div>
