...
<h1> Kategori Yönetimi<small>Kategoriler</small> </h1>
<ol class="breadcrumb">
        ...
        <table id="myTable" class="table table-bordered table-striped">
          ...
          <tbody>
  			 <?
			 	...
			 	$sayfada = 100;
				$sayfa = $_GET["sayfa"];
				$say = $db->query("SELECT * FROM category ORDER BY Id DESC");
				$toplam = $say->rowCount();
				$toplam_sayfa = ceil($toplam / $sayfada);
				if($sayfa < 1) $sayfa = 1;
				if($sayfa > $toplam_sayfa) $sayfa = $toplam_sayfa;
				$limit = ($sayfa - 1) * $sayfada;
				$sql = $db->query("SELECT * FROM category ORDER BY Id DESC LIMIT $limit,$sayfada");
				} else {
				$s = $_POST['s'];
				$sql = $db->query("SELECT * FROM category WHERE kategori_adi LIKE '%$s%' ORDER BY Id DESC");	
				}
				while ($a = $sql->fetch(PDO::FETCH_ASSOC)){
				$sql2 = $db->query("SELECT * FROM category WHERE Id = '{$a['ustkategoriId']}'");
				$b = $sql2->fetch(PDO::FETCH_ASSOC);
				$sql3 = $db->query("SELECT * FROM moduls WHERE Id = '{$a['modul']}'");
				$c = $sql3->fetch(PDO::FETCH_ASSOC);
				if ($c["name"] == ""){
				$c["name"] = "Belirtilmemiş";	
				}
				
				if ($b["kategori_adi"] == ""){
				$b["kategori_adi"] = "Anakategori";	
				}
				if ($a["tip"] == "0"){
					$tip = "İlan";
				} else {
				...
				echo '
				<tr>
                  <td>'.$a["kategori_adi"].'</td>
                  <td>'.$b["kategori_adi"].'</td>
                  <td>'.$c["name"].'</td>
				  <td>'.$tip.'</td>
                  <td><a class="btn btn-default btn-sm" href="index.php?page=kduzenle&id='.$a["Id"].'">Düzenle</a> <a class="btn btn-danger btn-sm" href="kategorisil.php?id='.$a["Id"].'" onclick="return sor()">Sil</a></td>
                </tr>
				';	
				}
			?>
