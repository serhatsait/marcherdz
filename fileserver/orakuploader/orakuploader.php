...
$main_path = $dirback.'files/'.$_SESSION["resimklasoru"].'';
$thumbnail_path = $dirback.'files/'.$_SESSION["resimklasoru"].'/thumbnail';
if(isset($_GET['delete']))
{
	...
	$del = $_GET['delete'];
	$sql2 = $db->prepare("DELETE FROM images WHERE ilanId = '{$idd}' and name = '$del'");
	$sql2->execute();	
	exit;
}
...
$idd = $_SESSION["resimklasoru"];
	$sql = $db->prepare('INSERT INTO images (Id,ilanId,name,s) VALUES (?,?,?,?)');
	$ekle = $sql->execute(array(null, $idd,$filename,0));
